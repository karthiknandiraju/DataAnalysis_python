...
for i in file_path : 
    with open(i) as f : 
        ...
        for w in f.readlines() : 
            try : 
                if safe_parse(w) : 
                    #print(safe_parse(w)['user']['id_str'])
                    #print(safe_parse(w)['text'])
                    data.append((safe_parse(w)['user']['id_str'], safe_parse(w)['text'] ))  
                    tweets.append(safe_parse(w)['text'])
            except KeyError : 
                print("error boy")
                
        #first = next(f).decode()
        #print(safe_parse(first)['text'])
...
# YOUR CODE HERE
users_count = rdd2.groupByKey().count()
#raise NotImplementedError()
        ...
        for w in f.readlines() : 
            try : 
                if safe_parse(w) : 
                    #print(safe_parse(w)['user']['id_str'])
                    #print(safe_parse(w)['text'])
                    total_users.append(safe_parse(w)['user']['id_str'])  
            except KeyError : 
                print("error boy")
...
for key, value in partition.items() : 
         partition_list.append((value,key))
         partision_users.append(key)  
...
# YOUR CODE HERE
rdd3 = sc.parallelize(partition_list).groupByKey().map(lambda k : (k[0], sum([posts[x] for x in k[1]])))
nonpartisan_users = []
for i in total_users : 
    if i not in partision_users and i not in nonpartisan_users : 
        nonpartisan_users.append(i)
non_posts = []
for i in nonpartisan_users : 
       non_posts.append(posts[i])
counts_per_partition = rdd3.sortByKey(True).collect()        
counts_per_partition.append((7,sum(non_posts)))
#raise NotImplementedError()
...
total_users_n = partision_users + nonpartisan_users
...
num_of_tokens = sc.parallelize(data).groupByKey().map(lambda x: (x[0],[k for k in x[1]]))


# In[29]:


texts = num_of_tokens.collect()
...
texts = sc.parallelize(texts).map(lambda x: ''.join(x[1])).collect()
...
token_frequency = Counter(toksas)
...
#token_frequency = {}
#for i in toksas : 
#    if i in token_frequency.keys() : 
#         token_frequency[i] = token_frequency[i] + 1
#    else : 
#         token_frequency[i] = 1 
